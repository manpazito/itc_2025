...
def _yes_blank_map(x):
    ...
    return {"Y": "Yes", "": "No", None: "No"}.get(
        x, "Yes" if str(x).upper() == "Y" else "No"
    )
...
SPECIAL_COND = {
    "1": "Schoolbus on Rdwy",
    "2": "State Univ / SFIA",
    "3": "Schoolbus off Rdwy",
    "4": "Offroad (Unimpr.)",
    "5": "Vista/Rest/Scale/Inspect",
...
BEAT_TYPE = {
    ...
    2: "CHP Cnty Line",
    3: "CHP Cnty Area",
    4: "Schoolbus City Rdwy",
    5: "Schoolbus not Rdwy",
    6: "Offroad (Unimpr.)",
    7: "Vista/Rest/Scale",
...
# Ordinal (cast to int)
COLLISION_SEVERITY = {1: "Fatal", 2: "Severe", 3: "Minor", 4: "Possible", 0: "PDO"}
...
PCF_VIOL_CATEGORY = {
    ...
    "15": "Brakes",
    "16": "Other Equip",
    "17": "Other Hazardous Viol",
    "18": "Other Than Driver/Ped",
    "19": "—",
    "20": "—",
    "21": "Unsafe Start/Backing",
...
MVIW = {
    ...
    "I": "Fixed Object",
    "J": "Other Object",
    "0": "Non-Collision + Add’l Obj",
    "1": "Ped + Add’l Obj",
    "2": "Other MV + Add’l Obj",
    "3": "MV Other Rdwy + Add’l Obj",
    "4": "Parked MV + Add’l Obj",
    "5": "Train + Add’l Obj",
    "6": "Bicycle + Add’l Obj",
    "7": "Animal + Add’l Obj",
    "8": "Fixed Obj + Add’l Obj",
    "9": "Other Obj + Add’l Obj",
    "-": NOT_STATED,
}
...
def apply_tims_mappings(df):
    """
    Return a copy of the DataFrame with crash code values replaced by readable labels.

    What this does:
    - Uses lookup tables to convert coded values into short, human-friendly labels.
    - Keeps string codes with letters or leading zeros intact.
    - Converts some numeric columns to integers where appropriate.
    - Converts 'Y or blank' columns into 'Yes'/'No'.
    """
    out = df.copy()

    # Replace codes using the lookup dictionaries
    for col, mapping in COLUMN_MAPS.items():
        if col not in out.columns:
            continue

        ser = out[col]
        if ser.dtype.kind in "ifc":  # numeric
            out[col] = (
                ser.map(mapping)
                .fillna(ser)
                .fillna(NOT_STATED)
